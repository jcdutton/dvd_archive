...
static int DVDReadLBUDF( struct dvd_reader_s *device, uint32_t lb_number,
...
static int GetUDFCache(struct dvd_reader_s *device, UDFCacheType type,
...
static int SetUDFCache(struct dvd_reader_s *device, UDFCacheType type,
                       uint32_t nr, void *data)
{
  int n;
  struct udf_cache *c;
 ...
 */
static int UDFMapICB( struct dvd_reader_s *device, struct AD ICB, uint8_t *FileType,
 ...
 * return 1 on success, 0 on error;
 */
static int UDFScanDir( struct dvd_reader_s *device, struct AD Dir, char *FileName,
...
static int UDFGetAVDP( struct dvd_reader_s *device,
                       struct avdp_t *avdp)
{
 ...
 */
static int UDFFindPartition( struct dvd_reader_s *device, int partnum,
...
uint32_t UDFFindFile( struct dvd_reader_s *device, char *filename,
                      uint32_t *filesize )
{
  uint8_t LogBlock_base[ DVD_VIDEO_LB_LEN + 2048 ];
  uint8_t *LogBlock = (uint8_t *)(((uintptr_t)LogBlock_base & ~((uintptr_t)2047)) + 2048);
...
#if 0
	printf("Partition: Valid=%d, VolumeDesc=%s, Flags=0x%x, Number=%d, Contents=%s, Accesstype=%d, Start=0x%x, Length=0x%x\n",
		partition.valid,
		partition.VolumeDesc,
		partition.Flags,
		partition.Number,
		partition.Contents,
		partition.AccessType,
		partition.Start,
		partition.Length);
#endif
...
int UDFGetPartition(struct dvd_reader_s *device, int *start, int *length) {
  ...
  tmp = UDFFindPartition(device, 0, &partition);
  if (tmp) {
 ...
 */
static int UDFGetDescriptor( struct dvd_reader_s *device, int id,
...
static int UDFGetPVD(struct dvd_reader_s *device, struct pvd_t *pvd)
{
 ...
 */
int UDFGetVolumeIdentifier(struct dvd_reader_s *device, char *volid,
 ...
 * or 0 on error
 */
int UDFGetVolumeSetIdentifier(struct dvd_reader_s *device, uint8_t *volsetid,
